...
import { recordLearningEvent, sendCustomerReply, takeoverCustomerMessage } from "@/shared/api/client";
import type { CustomerInboxItem } from "@/shared/api/types";
...
export function CustomerWorkbenchView({ inbox }: { inbox: CustomerInboxItem[] }) {
  ...
  const selectedDraft = selected ? draftOverrides[selected.id] ?? buildLocalDraft(selected) : "";
  ...
  async function handleSend() {
    ...
    const wasEdited = selectedDraft.trim() !== buildLocalDraft(selected);
    if (wasEdited) {
      const learning = await recordLearningEvent({
        ...
        action: "edited",
        original_content: buildLocalDraft(selected),
  ...
  async function handleRecordEdit() {
    ...
    if (!finalContent || finalContent === buildLocalDraft(selected)) {
